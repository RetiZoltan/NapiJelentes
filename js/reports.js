...
function _buildWrap(el) {
  ...
  // Nem flex `gap`-ot használunk a spacing-hez (html2canvas nem veszi figyelembe),
  // hanem explicit margin-t a gyerek elemeken.
  const dashcss = `.dash-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:14px;}.dash-widget{background:${LM.surf};border:1px solid ${LM.b};border-radius:12px;padding:18px 20px;}.dash-w-hdr{display:flex;align-items:center;margin-bottom:12px;}.dash-w-icon{font-size:22px;line-height:1;margin-right:8px;}.dash-w-title{font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:${LM.t3};}.dash-w-body{display:block;}.dash-w-value{font-size:30px;font-weight:700;color:${LM.t1};font-family:'Lora',serif;line-height:1.2;margin-bottom:4px;}.dash-w-value>*{margin-right:6px;}.dash-w-sub{font-size:12px;color:${LM.t3};}.g2{display:grid;grid-template-columns:1fr 1fr;gap:12px;}`;
  style.textContent = vars + css + rcss + hfix + dashcss;
